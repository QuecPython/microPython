...
void Helios_SaveImage(unsigned char* pYUVBuffer, int width, int height) {
	...
	if(image_info.img_width <= width && image_info.img_height <= height) {
	...
	if(callback_cur != NULL) {
		...
		if(mp_obj_is_callable(callback_cur)){
			mp_sched_schedule(callback_cur, MP_OBJ_FROM_PTR(mp_obj_new_list(2, save_cb)));
		}
	}
}
