...
STATIC mp_obj_t qpy_wifiscan_control(mp_obj_t option)
{
	...
	if ((opt != 0) && (opt != 1))
	{
		mp_raise_ValueError("invalid value, option should be 0 or 1.");
	}
...
STATIC mp_obj_t qpy_wifiscan_set_configuration(size_t n_args, const mp_obj_t *args)
{
	...
	{
		mp_raise_ValueError("invalid value, timeout should be in [4,255].");
	}

	if ((info.round < 1) || (info.round > 3))
	{
		mp_raise_ValueError("invalid value, round should be in [1,3].");
	}

	if ((info.max_ap_nums < 4) || (info.max_ap_nums > 30))
	{
		mp_raise_ValueError("invalid value, max_bssid_num should be in [4,30].");
	}

	if ((info.scan_time < 1) || (info.scan_time > 255))
	{
		mp_raise_ValueError("invalid value, scan_time_out should be in [1,255].");
	}

	if ((info.priority != 0) && (info.priority != 1))
	{
		mp_raise_ValueError("invalid value, priority should be in [0,1].");
	}
#else
    #ifdef PLAT_Unisoc
    if ((info.timeout < 120) || (info.timeout > 5000))
	{
		mp_raise_ValueError("invalid value, timeout should be in [120,5000].");
	}

	if ((info.round < 1) || (info.round > 10))
	{
		mp_raise_ValueError("invalid value, round should be in [1,10].");
	}

	if ((info.max_ap_nums < 1) || (info.max_ap_nums > 300))
	{
		mp_raise_ValueError("invalid value, max_bssid_num should be in [1,300].");
	}
...
STATIC mp_obj_t qpy_wifiscan_sync_start(void)
{
	...
	if (status == 0)
	{
		mp_raise_ValueError("WiFi-Scan is not enabled.");
	}
